#!/usr/bin/env python3
"""
Step 3 (US) — Compute CAC by product by day for Larroude US.
Same methodology as BR. All in USD (no conversion).

Output: cac_by_product_us.json
"""
...
# Produtos a ignorar (upsells, seguros, gift cards, etc — não são calçados)
EXCLUDE_KEYWORDS = [
...
new_customers_d = defaultdict(lambda: defaultdict(int))
revenue_d = defaultdict(lambda: defaultdict(float))
units_d = defaultdict(lambda: defaultdict(int))
total_revenue_d = defaultdict(float)
...
print("[US] Top 15 produtos por UNIDADES vendidas (28D):")
for i, pid in enumerate(top15_units, 1):
    print(f"  {i:2d}. {title_by_product[pid][:55]:55s}  units={units_by_product[pid]:5d}  CAC=${cac_28d(pid) if cac_28d(pid) else 0:>7,.0f}")

print(f"\n[US] Top 15 produtos com MENOR CAC (28D, min {MIN_NEW_CUSTOMERS} novos clientes):")
for i, pid in enumerate(top15_low_cac, 1):
    print(f"  {i:2d}. {title_by_product[pid][:55]:55s}  CAC=${cac_28d(pid):>7,.0f}  newCust={new_cust_by_product[pid]:4d}")
