#!/usr/bin/env python3
"""
Step 1 — Pull Shopify US orders for last 28 days via Admin GraphQL API.

Saves raw orders to orders_raw.json with the fields we need for CAC analysis:
- created_at, customer.numberOfOrders, customer.id
- line items: product_id, product_title, quantity, original_total

The "is new customer" flag = customer.numberOfOrders == 1 (first order ever).
"""
...
SHOP = os.environ["SHOPIFY_BR_STORE_DOMAIN"]
TOKEN = os.environ["SHOPIFY_BR_ADMIN_API_TOKEN"]
...
OUT_PATH = Path(__file__).parent / "orders_raw.json"
